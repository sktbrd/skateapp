...
const HiveStats = () => {
    ...
    const [delegatedHivePower, setDelegatedHivePower] = useState<string>("0"); // Separate state variable for delegated Hive Power
    ...
    const convertVestingSharesToHivePower = async (
        ...
        setDelegatedHivePower(delegatedHivePower.toFixed(3));
    };
    ...
    const fetchHiveStats = async () => {
        ...
        if (account && account.length > 0) {
            try {
                ...
                const hivePowerWorth = parseFloat(hivePower) * conversionRate; // Calculate Hive Power Worth
                const delegatedHivePowerWorth = parseFloat(delegatedHivePower) * conversionRate; // Calculate Delegated Hive Power Worth
