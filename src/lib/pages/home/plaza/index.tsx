...
const Plaza: React.FC = () => {
    ...
    const fetchComments = async () => {
        try {
            const allComments: CommentProps[] = await client.database.call('get_content_replies', [
                URLAuthor,
                URLPermlink,
            ]);
    ...
    const handleVote = async (comment: CommentProps) => {
        ...
        try {
            await voteOnContent(user.user.name, comment.permlink, comment.author, 10000);
        } catch (error: any) {
    ...
    const uploadFileToIPFS = async (file: File): Promise<string | null> => {
        try {
            ...
            if (file.type.startsWith("video/mp4") || file.type.startsWith("image/")) {
                ...
                console.error("Invalid file type. Only video files (MP4) and images are allowed.");
                return null;
            }
        } catch (error) {
            ...
            >
                ...
                >
                    ...
                    <Box style={{ display: 'flex', justifyContent: 'space-between', margin: '5px' }}>
                        ...
                        <label htmlFor="fileInput" style={{ cursor: "pointer", display: "inline-block", padding: "10px", border: "1px solid limegreen", borderRadius: "5px", backgroundColor: "limegreen", color: "white" }}>
                                ...
                                <Box key={comment.id} >
                                    ...
                                    <Box style={postContainerStyle}>
                                        <HStack justifyContent={"space-between"}>
                                            ...
                                            >
                                                <Badge marginBottom={"27px"} colorScheme="yellow" fontSize="sm">
                                                    {comment.total_payout_value !== 0 ? `${comment.pending_payout_value}` : "0.000 "}
                                                </Badge>
                                            </Tooltip>
                                        </HStack>
                                        <Divider />
