...
function ImageLoadBar_hook(ctx, width, height, total, current, image) {
	...
	// get parameters:
	var backgroundColor = getc("background_color", "#141932");
	var barBackgroundColor = getc("bar_background_color", "#E66464");
	var barForegroundColor = getc("bar_foreground_color", "#2192A9");
	var barBorderColor = getc("bar_border_color", "#BADA55");
	var barWidth = getf("bar_width", Math.round(width * 0.6));
	var barHeight = getf("bar_height", 40);
	var barBorderWidth = getf("bar_border_width", 5);
	var barOffset = getf("bar_offset", -50);
	// background:
