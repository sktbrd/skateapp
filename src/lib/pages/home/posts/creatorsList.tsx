...
import Comments from "./postModal/comments";
  ...
  type Post = {
    ...
    title: string;
    body: string;
    user: string; // Add the missing "user" property
    weight: number; // Add the missing "weight" property
    earnings: number;
  };
  
  const GnarsBlog = ({ tags }: GnarsBlogProps) => {
    ...
    const fetchPosts = async () => {
      ...
      try {
        ...
        const postsWithThumbnails: Post[] = result.map((post) => {
        ...
        const updatedPostsWithEarnings: Post[] = await Promise.all(
